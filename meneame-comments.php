...
function meneame_comments__get_vardefault( $varName = '' ) {
	...
	switch($varName) {
		case "defaults":
			$varValue = array(
				'rss_url' => 'https://www.meneame.net/comments_rss2.php',
				'rss_param_id' => 'id',
...
function meneame_comments__get_option( $name ) {
	...
	if ( isset($meneame_comments[$name]) ) {
		...
		/* Patch ( < 0.0.19 ) */
		if ( $name == 'rss_url' AND $meneame_comments[$name] == 'https://meneame.net/comments_rss2.php' )
			$meneame_comments[$name] = 'https://www.meneame.net/comments_rss2.php';
...
function meneame_comments__posts_trackbacked( $post_id = '' ) {
	...
	return $wpdb->get_results("
		...
		FROM ". $wpdb->comments ."
		WHERE 1 = 1
			AND ( comment_author_url LIKE 'https://meneame.net/story/%' OR comment_author_url LIKE 'https://www.meneame.net/story/%' )
...
function meneame_comments__check_trackback( $tb_id ) {
	...
	if ( $commentdata['comment_type'] != 'trackback' AND $commentdata['comment_type'] != 'pingback' )
		return false;
	...
	$dupe = "
		...
		WHERE 1 = 1
			...
			AND (
				...
				". ( ( $comment_author_email ) ? " OR comment_author_email = '".$commentdata['comment_author_email']."'" : "" ) ."

			)
			AND comment_author_url = '".$commentdata['comment_author_url']."'
		LIMIT 1
	";
...
add_action('pingback_post', 'meneame_comments__trackback_post');
...
function meneame_comments__without_meneame( $args = '' ) {
	...
	$sqlTypes = array();;
	if ( $args['comments'] ) $sqlTypes[] = ' comment_type = "comment" ';
	...
	$without_meneame = $wpdb->get_results("
		...
		FROM ". $wpdb->comments ."
		WHERE 1 = 1
			AND comment_post_ID = '".$args['post_id']."'
			AND comment_approved = '1'
